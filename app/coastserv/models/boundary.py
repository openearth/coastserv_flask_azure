...
class Boundary(object):
    ...
    def write_bc_file(self, sub, bnd):
        ...
        if len(data_list) == 0:
            ...
            if os.path.exists(os.path.join(self.dir, '..', 'bc.zip')):
                mode='a'
            else: mode='w'

            with zipfile.ZipFile(os.path.join(self.dir, '..', 'bc.zip'), mode) as zf:
                fullpath = os.path.join(self.dir, '%s_%s.bc' % (sub, bnd))
    ...
    def write_new_ext_file(self):
        ...
        with zipfile.ZipFile(os.path.join(self.dir, '..', 'ext.zip'), mode) as zf:
